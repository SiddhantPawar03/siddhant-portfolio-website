...
const About = () => {
  const getInfoData = (title: string) => {
    return infoData.find((item) => item?.text === title)!;
  };

  const getQualificationData = (title: string) => {
    return qualificationData.find((item) => item.title === title)!;
  };

  const getSkillData = (title: string) => {
    return skillData.find((item) => item.title === title)!;
  };

  const getAchievementData = (title: string) => {
    return achievementData.find((item) => item.title === title)!;
  };
                ...
                <TabsContent value="personal">
                  <div className="text-center xl:text-left">
                    <h3 className="h3 mb-4">
                      Ready to deliver unmatched service quality from day one!
                    </h3>
                    <p className="subtitle max-w-xl mx-auto xl:mx-0">
                      Self-motivated individual and driven technology
                      professional with comprehensive accomplishments.
                      Demonstrated success developing and seamlessly executing
                      plans in complex organizational structures. Skilled in
                      Communication, Problem Solving, Leadership, Team
                      Management, C++ (Programming Language) and Python
                      (Programming Language). Strong engineering professional
                      with a Bachelor of Technology - BTech focused in Computer
                      Engineering from Vishwakarma Institute Of Technology.
                    </p>
                    ...
                    <div className="grid md:grid-cols-2 gap-y-8">
                      <div className="flex flex-col gap-y-6">
                        <div className="flex gap-x-4 items-center text-[22px] text-primary">
                          ...
                          <h4 className="capitalize font-medium">
                            {getQualificationData("experience")?.title}
                          </h4>
                        </div>
                        <div className="flex flex-col gap-y-8">
                          {getQualificationData("experience")?.data?.map(
                            (item, index) => {
                      ...
                      <div className="flex flex-col gap-y-6">
                        <div className="flex gap-x-4 items-center text-[22px] text-primary">
                          ...
                          <h4 className="capitalize font-medium">
                            {getQualificationData("education")?.title}
                          </h4>
                        </div>
                        <div className="flex flex-col gap-y-8">
                          {getQualificationData("education")?.data?.map(
                            (item, index) => {
                ...
                <TabsContent value="skills">
                  <div className="text-center xl:text-left">
                    ...
                    <div className="mb-16">
                      ...
                      <div>
                        {getSkillData("skills")?.data?.map((item, index) => {
                          const { name } = item;
                          return (
                            <div
                    ...
                    <div>
                      ...
                      <div className="flex gap-x-8 justify-center xl:justify-start">
                        {getSkillData("tools")?.data?.map((item, index) => {
                          const { imgPath } = item;
                          return (
                            <div key={index}>
                              {imgPath && (
